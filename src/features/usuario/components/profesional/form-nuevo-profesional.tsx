...
//features
import { UsuarioService, FormNuevoProfesionalInterface } from '@/features';
//shared
...
export function FormNuevoProfesional() {
    ...
    } = useForm<FormNuevoProfesionalInterface>()

    const onSubmit: SubmitHandler<FormNuevoProfesionalInterface> = async (data) => {
        try{
            await UsuarioService.createProfesional(data);
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="w-[90%] m-auto">
        ...
        {errors.correo && <span className="text-red-700 text-sm">{errors.correo.message}</span>}
        ...
        {errors.fecha_nacimiento && <span className="text-red-700 text-sm">{errors.fecha_nacimiento.message}</span>}

        {/* password */}
        <InputOutlined label="Contraseña" name="password" type="password" register={register} rules={{ required: "Campo requerido", pattern: {value: /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]{8,16}$/, message: "Debe contener al menos una mayúscula, una minúscula, un número y un carácter especial"}, minLength: {value: 8, message: "Debe tener al menos 8 caracteres"}, maxLength: {value: 16, message: "Debe tener máximo 16 caracteres"}}}/>
        {errors.password && <span className="text-red-700 text-sm">{errors.password.message}</span>}

        {/* confirmar password */}
        <InputOutlined label="Confirmar contraseña" name="password_confirmation" type="password" register={register} rules={{required: "Debe confirmar contraseña", validate: (value: any) => value === watch("password") || "Las contraseñas no coinciden" }}/>
        {errors.password_confirmation && <span className="text-red-700 text-sm">{errors.password_confirmation.message}</span>}
