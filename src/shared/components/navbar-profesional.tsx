...
export function NavbarProfesional() {
  ...
  const navList = navInfo.map((info, index) => (
    <li key={index}>
      <Link
        href={info.ruta}
        className="flex items-center py-3 px-4 text-primary rounded-lg font-medium hover:bg-tertiary"
        onClick={() => setOpen(false)}
      >
        <FontAwesomeIcon icon={info.icon} className="me-2 text-green-600" style={{width: '16px', height: '16px'}}/>
  ...
  return (
    <div className="">
      ...
      <div className="flex md:hidden items-center justify-between bg-white border-b border-gray-300 px-4 py-3 fixed top-0 left-0 right-0 z-30">
        ...
        <button
          className="p-2 rounded focus:outline-none focus:ring-2 text-green-600 focus:ring-green-500"
          onClick={() => setOpen((o) => !o)}
