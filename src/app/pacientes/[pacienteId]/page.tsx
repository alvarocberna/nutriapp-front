...
export default function Paciente(){
    ...
    return(
        <ProfessionalCont>
            ...
            <ContSubSec>
                <HeadSubSec>
                    <h2 className="w-[50%] left-[19px] top-[24px] text-primary font-inter text-[22px] font-bold">
                        Sesiones anteriores
                    </h2>
                    <div className='w-[50%] sm:w-[25%] h-[40px]'>
                ...
                <DivSubSec/>
                <div className="h-[40px] w-full text-gray-600 text-md flex justify-center items-center ">
    ...
    return(
        <RowSubSec>
            <div className="flex w-full justify-between items-center text-sm">
                <span className="w-[20%] text-md text-black text-start">{nro_consulta}°</span>
                <span className="w-[20%] text-md text-black text-center">{fechaConsulta}</span>
                {/* <span className="w-[20%] hidden sm:flex text-md text-black text-center">{peso} Kg</span> */}
                <div className="w-[60%] sm:w-[40%] flex justify-end">
                    <Link href={`/pacientes/${id_paciente}/${id_consulta}/resultados`} className="h-[30px] w-[30px] flex justify-center items-center bg-green-600 mx-2 rounded-[3]">
                        <FontAwesomeIcon icon={faEye} className=" text-white" style={{width: '16px', height: '16px'}}/>
                    </Link>
                    <Link href={`/pacientes/${id_paciente}/${id_consulta}/modificar`} className="h-[30px] w-[30px] flex justify-center items-center bg-amber-400 mx-2 rounded-[3]">
                        <FontAwesomeIcon icon={faPencil} className=" text-white" style={{width: '16px', height: '16px'}}/>
                    </Link>
                    <Link href="#" onClick={() => setOpen(true)} className="h-[30px] w-[30px] flex justify-center items-center bg-red-400 mx-2 rounded-[3]">
                ...
                <Modal isOpen={open} onClose={() => setOpen(false)}>
                    <div className="flex flex-col">
                        ...
                        <button onClick={() => {deleteConsulta()}} className="w-1/2 m-auto bg-red-400 py-2 text-white rounded">
                            Borrar
                        </button>
                    </div>
                </Modal>
